...
export default function StickyExploreBar() {
  return (
    <>
      ...
      >
        <div className="mx-auto grid w-full max-w-3xl grid-cols-2 overflow-hidden rounded-2xl shadow-[0_-12px_40px_rgba(235,216,16,0.4)] sm:rounded-3xl">
          {/* Courses */}
          <a
            href={COURSES_URL}
            onClick={() => track("sticky_explore_courses_click", { page: "home", destination: "courses" })}
            className="group flex min-h-[68px] items-center justify-between gap-2 border-r border-slate-900/20 bg-[#ebd810] px-3 py-3 text-left text-slate-900 transition-colors hover:bg-amber-300 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-inset focus-visible:ring-slate-900/30 sm:min-h-[76px] sm:px-5"
          >
            <span className="flex min-w-0 items-center gap-2 sm:gap-3">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-900/15 sm:h-11 sm:w-11">
                <BookOpen
                  aria-hidden="true"
                  className="h-[18px] w-[18px] text-slate-900 sm:h-5 sm:w-5"
                />
              </span>

              <span className="min-w-0">
                <span className="hidden text-[10px] font-semibold uppercase tracking-[0.12em] text-amber-900 sm:block sm:text-xs">
                  Self-paced learning · from ₹511
                </span>

                <span className="block text-[13px] font-extrabold leading-tight text-slate-900 sm:mt-0.5 sm:text-base lg:text-lg">
                  Explore Courses
                </span>
              </span>
            </span>
            ...
            <ChevronRight
              aria-hidden="true"
              className="hidden h-5 w-5 shrink-0 text-slate-900 transition-transform duration-200 group-hover:translate-x-1 md:block"
            />
          </a>
